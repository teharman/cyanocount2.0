...
new_base_dir<-path("models/ID_predict_model")
...
save_model_tf(new.model, "models/ID_predict_model/model/") #save model here
test_model<-load_model_tf('models/ID_predict_model/model/', custom_objects = NULL, compile = TRUE)
result<-evaluate(test_model,test_dataset)

model_label<-dir("misc/Train/")
test <- ("models/ID_predict_model/test/Anabaena/")
img <- image_read("models/ID_predict_model/test/Anabaena/Anabaena_20X_color (492).png")
ras <- as.raster(img)
mat <- as.matrix(ras)
test_image1 <- Image(mat,colormode = Color)
display(test_image1)
resize_img<-resize(test_image1,w=100,h=100)
x <- resize_img%>%array_reshape(.,c(1,dim(.)))%>%'/'(255)
