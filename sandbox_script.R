...
display(st_blob)
st_img <- stackObjects(cmask,grey_imgs[[1]])
display(st_img)


####Create features and export images####
features.data.blob<-computeFeatures(cmask,img_watershed)
features.data.img<-computeFeatures(cmask,grey_imgs[[1]])
...
#blob st img save
Index_blob<-paste0(sub(".tif", replacement = "blob_", x=imgNames[[1]]))
Index1<-paste0(sub(".tif", replacement = "/ ", x=imgNames[[1]]))
...
for(k in 1:dim(st_blob)[3]) {
  ...
  analyzed_image1<-paste0(sub(".tif", replacement = " ", x=imgNames[[1]]),"_frame")
...
#grey st img save
Index_grey<-paste0(sub(".tif", replacement = "grey_", x=imgNames[[1]]))
Index1<-paste0(sub(".tif", replacement = "/ ", x=imgNames[[1]]))
newpath<-file.path(image_savdir,Index1)
if(!dir.exists(newpath)){
  dir.create(newpath)
